...
def columns_process(category, df, original_columns_Dict):
    ...
    df_new = df_new.drop(df_new.columns.difference(list(original_columns_Dict.keys())), axis = 1)
...
for k, v in files_path_Dict.items():
    df_Dict[k] = pd.read_csv(v, encoding='utf-8', quotechar='"', na_values=['s/d', 'nan'])
...
df_categoria.rename(columns = {"categoría" : "referencia", "size" : "registros"}, inplace = True)
...
df_prov_categoria.insert(loc = 0, column = "referencia", value = df_prov_categoria[['provincia', 'categoría']].agg('_'.join, axis=1))
...
df_lugares_cines["cantidad de espacios INCAA"] = df_lugares_cines["cantidad de espacios INCAA"].str.lower()
df_lugares_cines["cantidad de espacios INCAA"] = df_lugares_cines["cantidad de espacios INCAA"].map({'0': 0, 'si': 1})
logging.debug(df_lugares_cines["cantidad de espacios INCAA"].unique())

df_lugares_cines["cantidad de espacios INCAA"] = pd.to_numeric(df_lugares_cines["cantidad de espacios INCAA"]).astype('Int64')
